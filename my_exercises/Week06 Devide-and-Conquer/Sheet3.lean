...
-- Sometimes, we cannot immediately call funtional induction even though the algorithm is recusive.
...
theorem subinterval_to_interval_qlt {n : ℕ} (arr : SortedArrayFun n) (q a mid b: ℕ)
  ...
  · intro h'
    ...
    replace hi: arr.get i = q := by simp [hi]
    rw [← hi] at h_q
...
-- # `contains_bs` itself does not allow induction/cases on the two interval bounds `a` and `b`
#check contains_bs
-- # `contains_bs.bs_aux` allows induction/cases on the two interval bounds `a` and `b`
